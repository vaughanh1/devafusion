...
- **No Sensitive Data in Log Entries:** Every entry under `src/web/features/log/entries/` defaults to `visibility: "public"` and is rendered on the live site — treat its contents as published the moment it's committed, not as an internal note. Before proposing or writing a log entry, scrub every field (`summary`, `decisions`, `milestones`, `validation`) for: Azure resource IDs/GUIDs (role definition IDs, subscription/tenant IDs, object IDs), IP addresses, real hostnames/FQDNs beyond what's already public elsewhere on the site, connection strings, and any value that would help an attacker map this subscription's internal structure. Referring to a resource by its human-readable role/purpose ("the pipeline's custom Terraform deployment role") is always sufficient — the GUID or identifier is never needed to make the entry useful. This applies retroactively too: if a past entry is found to contain one of these, scrub it in the same change that found it, not as a deferred follow-up.
...
- **Zero Hardcoded Secrets:** It is strictly forbidden to commit any secret, API key, credential, or otherwise sensitive value at any time, including placeholder-looking values in non-production configuration. All secrets must be sourced from an approved, external store (Azure Key Vault, Azure DevOps Library variable groups) at build or run time. A violation of this rule requires an immediate and complete rotation of the exposed secret. This is enforced automatically — see Section 5a, Automated Secret Scanning. This extends to comments anywhere in the codebase (`.tf`, `.ts`, `.tsx`, pipeline YAML, README/docs) — never paste an Azure resource ID/GUID (role definition ID, subscription/tenant/object ID), IP address, or connection string into a comment "for reference," even one explaining a past incident. Gitleaks does not reliably catch a bare GUID or IP the way it catches a credential-shaped string, so this is not fully backstopped by the automated scan in Section 5a — it depends on this rule being followed at write-time. Describe the resource by its role/purpose instead (e.g. "the pipeline's custom Terraform deployment role"), exactly as required for log entries under Log-Driven Development (Section 3).
...
- **No Destructive `az` CLI Commands Against Live Resources — Ever, Without Explicit Per-Command Approval:** Any Azure CLI command that deletes, purges, or otherwise mutates a real resource's data or configuration (`az * delete`, `az * purge`, `az role definition delete/update`, `az * update` against a live resource, etc.) is strictly forbidden from being run unilaterally, in any mode, even when investigating an incident and even when the command appears reversible. Read-only inspection commands (`az * show`, `az * list`) are always permitted for diagnosis. Before running any destructive command: (1) state exactly what will be deleted/changed and why, (2) state how confident that reasoning actually is — an assumption inferred from a resource's *name* is not verification, only checking the authoritative source (e.g. `terraform state list`, the actual pipeline apply log) is, (3) get explicit user approval for that specific command, not a general "go ahead" on the surrounding task. This rule exists because a live Azure resource (a firewall rule, in one real incident) was deleted on an unverified guess about its origin, based on its name looking auto-generated, without checking whether it had a legitimate purpose or asking first — it turned out to have been created by the very pipeline apply already in progress. If a command needs to be run, prefer the export-edit-reapply pattern already established for role definition changes (`az * list -o json > file`, edit the file, `az * update @file`) over inline mutation, so there is always a reviewable diff before the mutating command runs.
