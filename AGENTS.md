...
- **Commits:** Write clean, atomic Git commits following the **Conventional Commits** specification (e.g., `feat(auth): ...`, `fix(db): ...`, `chore(ci): ...`).
...
- Once local validation passes perfectly, stage your files and push the branch to origin.
- Use the GitHub CLI (`gh pr create`) to open a formal Pull Request against `develop`.
