...
export const entry: LogEntry = {
  ...
  summary:
    "Wired pipelines/cd/web.yml to actually apply drizzle-kit migrations to the live psql-devafusion-dev-uks server - a temporary named firewall rule for the CD agent, manual approval on a dedicated Azure DevOps environment, and the current migration SQL printed in CI before that approval gate is ever reached. Went through two design passes in one sitting: first split detection/preview into a separate stage using a cross-stage output-variable condition whose exact syntax couldn't be verified against an authoritative Microsoft example, then discovered the git-diff-based gate itself was unsound (a single rejected/failed CD run would permanently and silently lose track of a pending migration) and removed it entirely in favour of running drizzle-kit migrate unconditionally, relying on its own idempotency.",
